...
export function PersonaOnboarding({ onDone }: PersonaOnboardingProps) {
  ...
  if (step === -1) {
    return (
      <Shell>
        ...
        <p className="mt-gap-sm max-w-2xl text-body-lg text-muted">
  ...
  return (
    <Shell>
      <p className="text-body text-muted">{progress}</p>
      <h2 className="mt-2 text-title font-bold leading-tight text-ink">
        {questionLabel}
      </h2>
      ...
      <div className="mt-gap">
        {q.kind === "text" ? (
          <textarea
            ...
            rows={3}
            className="block w-full max-w-2xl rounded-tile border-4 border-border bg-canvas px-6 py-4 text-body-lg text-ink shadow-inner focus:border-ink focus:outline-none"
            autoFocus
          />
        ) : (
...
function Shell({ children }: { children: React.ReactNode }) {
  return (
    <div
      role="dialog"
      aria-modal="true"
      className="fixed inset-0 z-50 overflow-y-auto bg-canvas px-10 py-10"
    >
...
function PrimaryButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="min-h-tile-min rounded-tile border-4 border-ink bg-ink px-10 py-4 text-label-lg text-canvas shadow-tile active:shadow-tile-pressed"
    >
      {children}
    </button>
  );
...
function SecondaryButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="min-h-tile-min rounded-tile border-4 border-border bg-soft px-10 py-4 text-label-lg text-ink shadow-tile active:shadow-tile-pressed"
    >
      {children}
    </button>
  );
